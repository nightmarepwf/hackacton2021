...
const EventsListManager = ({events, createEventButtonHandler}) => {
    return (
        <div>
            ...
            {events.map(item => (
                <div key={item.ID} style={{borderColor: "#0077b6", borderWidth: "2px", borderRadius: 5, padding: "10px", borderStyle: "solid", display: "flex", flexDirection: "row", marginBottom: 10}}>
                    ...
                    <div style={{display: "flex", flexDirection: "column" }} key={item.ID}>
                        ...
                        <span style={{fontSize: "1.2rem", fontWeight: 500, marginBottom: 10}}><b>Дата: </b>{new Date(item.event_date).toLocaleDateString()}</span>
